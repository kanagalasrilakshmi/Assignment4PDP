...
public class FlexibleControllerImpl implements Controller{
   ...
   */
  public FlexibleControllerImpl(Portfolio thePortfolio, View theView, InputStream in) {
      ...
      switch (in.next()) {
        ...
        case "C":
          ...
          while (!done) {
            ...
            switch (in.next()) {
              case "S":
                done = savePortfolio(objList, pfName, this.rootDir);
                break;
              case "Y":
  ...
  public boolean savePortfolio(ArrayList<Object> objList, String pfName, String path) {
    boolean done = false;
  //check if object list is empty nothing to save
    if (objList.size() == 0) {
      theView.showString("Portfolio must contain at least one entry!! ");
    } else {
      thePortfolio.createPortfolio(path, pfName, objList);
      done = true;
